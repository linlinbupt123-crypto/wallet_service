...
func (s *WalletService) DeriveNewAddress(ctx context.Context, userID, chainName, passphrase string) (string, error) {
	...
	switch chainName {
	case "btc":
		addr, err = s.BtcChain.DeriveAddress(seed, path)
	case "eth":
...
func (s *WalletService) GetBalance(ctx context.Context, chainName, address string) (string, error) {
	switch chainName {
	case "btc":
		return s.BtcChain.GetBalance(ctx, address)
	case "eth":
...
func (s *WalletService) SendTransaction(ctx context.Context, chainName, fromAddress, toAddress, amount string, passphrase string, userID string) (string, error) {
	...
	case "eth":
		...
		return s.EthChain.SendTransaction(ctx, seed, path, fromAddress, toAddress, amount)
	default:
		return "", errors.New("unsupported chain")
	}
}
